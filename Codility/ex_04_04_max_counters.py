...
************************************************************************************************************
The straight-forward solution, even though quite fast and elegant, involves updating all the
counters every time you get an update-all instruction.  This is the difference between a 60% score and a 100% score.

To avoid this hit you have to track the low and high watermarks and refer to them as you update individual counters.
...
def solution(N, A):
    ...
    """
    counters = [0] * (N + 1)  # one longer so the counter number works as an index without being off by one
    maximum = 0
    minimum = 0
    ...
    for key in A:
        if key <= N:
            # align with the minimum (avoid looping through all every time)
            ...
            # update minimum for update-all instruction operation
            minimum = maximum
