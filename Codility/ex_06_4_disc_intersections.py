...
A zero-indexed array A of N non-negative integers, specifying the radiuses of the
...
have at least one common intersecting discs. The function should return -1 if the number of intersecting
pairs exceeds 10,000,000. Given array A shown above, the function should return 11, as explained above.
...
# Analysis
The fast solution reliest on the formulation that counts the number of intersections:
    `intersections += istart - iend - 1`
You need to add the number of discs opened between the closing of this one and the last,
but don't try to increment by ones inside the while loop-that gets messy-and
do snip one off the total increment amount (not the total total for eg).
"""
...
def fast_solution(A):
    ...
    # every time a disc closes we add an intersection for every disc
    # that has opened since the last close
    intersections = istart = 0
    for idx in range(len(ends)):  # for every closing
        while istart < len(starts) and starts[istart] <= ends[idx]:  # step through unreconciled openings
            istart += 1
        # record intersections
        intersections += istart - idx - 1
